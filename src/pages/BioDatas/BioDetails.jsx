...
const BioDetails = () => {
    ...
    useEffect(() => {
        ...
        axiosSecure
            ...
            .then((res) => {
                ...
                axiosSecure
                    .get(
                        `/similar-biodata/${res.data?.biodataType}?exclude=${res.data?._id}`
                    )
                    .then((res) => setSimilarBiodatas(res.data || []))
                    .catch((err) => console.error("Failed to load similar biodata", err));
            })
    ...
    return (
        <div className="max-w-5xl mx-auto p-8 mt-12">
            ...
            {/* Main Biodata Card */}
            <div className="bg-gradient-to-r from-purple-500 via-pink-500 to-yellow-500 rounded-3xl shadow-xl text-white"></div>
                ...
                <div className="md:w-2/3 p-10 space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-gray-700 text-lg">
                        <div className="space-y-2">
                            <p><span className="font-semibold text-gray-900">Date of Birth: </span>{biodata?.dob}</p>
                            <p><span className="font-semibold text-gray-900">Age: </span>{biodata?.age} years</p>
                            <p><span className="font-semibold text-gray-900">Height: </span>{biodata?.height} cm</p>
                            <p><span className="font-semibold text-gray-900">Weight: </span>{biodata?.weight} kg</p>
                            <p><span className="font-semibold text-gray-900">Occupation: </span>{biodata?.occupation}</p>
                            <p><span className="font-semibold text-gray-900">Race: </span>{biodata?.race}</p>
                        </div>

                        <div className="space-y-2">
                            <p><span className="font-semibold text-gray-900">Father's Name: </span>{biodata?.fatherName}</p>
                            <p><span className="font-semibold text-gray-900">Mother's Name: </span>{biodata?.motherName}</p>
                            <p><span className="font-semibold text-gray-900">Permanent Division: </span>{biodata?.permanentDivision}</p>
                            <p><span className="font-semibold text-gray-900">Present Division: </span>{biodata?.presentDivision}</p>
                            <p><span className="font-semibold text-gray-900">Expected Partner Age: </span>{biodata?.expectedPartnerAge} years</p>
                            <p><span className="font-semibold text-gray-900">Expected Partner Height: </span>{biodata?.expectedPartnerHeight} cm</p>
                            <p><span className="font-semibold text-gray-900">Expected Partner Weight: </span>{biodata?.expectedPartnerWeight} kg</p>
                        </div>
                    </div>
