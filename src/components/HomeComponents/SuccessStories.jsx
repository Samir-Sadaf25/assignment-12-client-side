            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                {sortedStories.map(
                    ({ _id, coupleImageLink, submittedAt, rating, successStory }) => (
                        <div
                            key={_id}
                            className="bg-gray-50 rounded-xl p-6 shadow-md hover:shadow-xl transform hover:scale-[1.03] transition-all duration-300 flex flex-col items-center text-center"
                        >
                            <img
                                src={coupleImageLink}
                                alt="Couple"
                                className="w-32 h-32 rounded-full object-contain border-2 border-black mb-6 shadow-md"
                            />
